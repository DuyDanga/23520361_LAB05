...
BEGIN
    ...
    WHERE cgn.MaKyNang = @MaKyNang  -- Expert has the required skill
    AND cgn.CapDo >= @YeuCauCapDo   -- Expert's proficiency meets the requirement
    AND cg.NamKinhNghiem >= 5        -- Example: Experts must have at least 5 years of experience
    AND NOT EXISTS (                -- Prevent assigning the same expert to the project if already assigned
        SELECT 1
...
BEGIN
    ...
    -- Get the expert ID from the inserted record
    SELECT @MaChuyenGia = MaChuyenGia
    FROM inserted;

    -- Update the status of the expert to 'bận' (busy)
    UPDATE ChuyenGia
    SET TrangThai = N'Bận'
    WHERE MaChuyenGia = @MaChuyenGia;
END;
